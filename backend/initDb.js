...
async function initDatabase() {
    try {
        ...
        `);
        console.log("Problems table successfully recreated with structural JSONB metadata support.");
        ...
        for (const prob of sampleProblems) {
            ...
            await query(`
                ...
                ON CONFLICT (id) DO UPDATE SET
                    title=$2, description=$3, input_example=$4, output_example=$5,
                    template_python=$6, template_cpp=$7, template_java=$8, metadata_json=$9
            `, [prob.id, prob.title, prob.description, prob.input, prob.output, templatePy, templateCpp, templateJava, metadataJson]);
        }
        console.log("🚀 Successfully compiled and seeded all 10 problems with templates for Python, C++, and Java!");

        // 4. Re-create and seed your test cases table
        await query(`DROP TABLE IF EXISTS test_cases CASCADE`);
        await query(`
        ...
        const testCases = [
            ...
            ["palindrome-number", "10", "false"],
            ["fizz-buzz", "3", "['1','2','Fizz']"],
            ["reverse-string", "['h','e','l','l','o']", "['o','l','l','e','h']"],
            ["valid-parentheses", "'()[]{}'", "true"],
            ["merge-sorted-lists", "[1,2,4]\n[1,3,4]", "[1,1,2,3,4,4]"],
            ["max-subarray", "[-2,1,-3,4,-1,2,1,-5,4]", "6"],
            ["climbing-stairs", "2", "2"],
        ...
        console.log("🎯 Database Initializer finished successfully.");
        process.exit(0);
